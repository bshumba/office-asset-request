...
@section('content')
        ...
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-3xl space-y-4">
                ...
                <h2 class="text-3xl font-extrabold tracking-tight text-slate-950 sm:text-4xl">
                    Stay ahead of stock pressure before it slows down fulfilment.
                </h2>
            ...
            <div class="rounded-[28px] border border-white/60 bg-white/75 p-5 shadow-sm backdrop-blur">
                ...
                @can('reports.export')
                    <div class="mt-5">
                        <a href="{{ route($routePrefix.'.low-stock.export', request()->query()) }}" class="secondary-button w-full justify-center">
                            Export CSV
                        </a>
                    </div>
                @endcan
    ...
    <section class="grid gap-6 xl:grid-cols-[0.82fr_1.18fr]">
        ...
        >
            @if ($assets->isEmpty())
                <x-ui.empty-state
                    title="No low-stock assets matched these filters"
                    description="Reset the filters to review every asset currently at or below its reorder level."
                    action-label="Reset Filters"
                    action-url="{{ route($routePrefix.'.low-stock') }}"
                />
            @else
